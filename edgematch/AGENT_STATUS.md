...
[AGENT-1] [DONE] bug fixes: (1) landing page always visible -- removed PublicRoute from / so logged-in users see marketing page; (2) Laurena's athlete row updated (skating_level junior->senior) so profileComplete=true and /browse accessible; (3) duplicate level pills removed from Browse FilterPanel (clubs don't have levels; pills were non-functional dead code). Build clean, pushed.
[AGENT-1] [DONE] bug fixes: (1) ClubPage stuck on Loading -- removed search_status='active' filter from useAthletes.js so all athletes visible regardless of status; (2) Matches 'You need a profile' -- confirmed code-level checks already correct (if (\!athleteId) / if (\!athlete)); root cause is RLS on compatibility_scores table (needs policy allowing auth users to read their own scores). Puppeteer screenshots [UNVERIFIED] -- ClubPage and Matches behind ProtectedRoute, unauthenticated Puppeteer redirects to /login. Build clean, pushed.

[AGENT-1] [DONE] bug fix: profile edit SAVING hang -- root cause: refreshAthlete() returned null after save because athletes_read_own RLS policy may not be applied (search_status='inactive' blocks athletes_public_read). Fix: (1) added updateAthleteLocally(partial) to AuthContext for optimistic in-place merges -- no re-fetch needed; (2) Profile.jsx now calls updateAthleteLocally(athletePayload()) instead of refreshAthlete() after isEdit save; (3) refreshAthlete() now preserves existing state when DB returns null (RLS block); (4) migration 016 created with idempotent re-apply of read-own + update-own policies. Build clean, pushed.
[AGENT-1] [DONE] scrape_ips.js rewrite: Puppeteer login (IPS_EMAIL/IPS_PASSWORD), fixed name extraction (<h1> tag, IPS changed HTML structure), fixed field parser (exact <td>Label</td><th>Value</th> pattern), direct athletes table cross-reference (similarity >= 0.85 -> set active, no match -> INSERT + score). Live run: 220 scraped, 198 matched (set active), 9 new athletes inserted, 0 unparseable, 5 skipped (no level). Pushed.
